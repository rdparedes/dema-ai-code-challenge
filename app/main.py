...
async def update_product(product_id: str, new_data: ProductIn_Pydantic):
    await Product.filter(product_id=product_id).update(**new_data.model_dump(exclude_none=True))
    return await Product_Pydantic.from_queryset_single(Product.get(product_id=product_id))
